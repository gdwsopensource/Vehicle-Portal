 ...
 */
@RestController
public class CrossPoliceController {
	@Autowired
	private Systemconfigs conf;

	@RequestMapping("getCrossPolice")
	public JSONObject getCrossPolice(String crossId,String crossDate) {
		String url = "http://" + conf.getDomain() + ":" + conf.getPort();
		return JSONObject.parseObject(
				HttpClientUtils.doGet(url + "/policeDistribution?crossDate=" + crossDate+"&crossId="+crossId, null));
	}
}
